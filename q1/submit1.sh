...
# Submit the second job (predict.sh) dependent on the successful completion of the first job
jobId2=$(sbatch --dependency=afterok:$jobId1 --output="slurm_out/slurm-%A_%a.out" --error="slurm_out/slurm-%A_%a.err" run_R.sh)
jobId2=$(echo $jobId2 | sed 's/Submitted batch job //')

# Optionally echo job IDs if needed for logging or further processing
